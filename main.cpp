...
#include "cs1037lib-button.h" // for basic buttons and controls
...
int main()
{
	...
	cout << " click 'X'-box to close the application \n\n " << endl; 
	  ...
	  // initializing buttons/dropLists and the window (using cs1037lib methods)
	int blank = CreateTextLabel(""); // adds grey "control panel" for buttons/dropLists, see "cs1037lib-buttons.h"
    SetControlPosition(blank,0,0); SetControlSize(blank,1280,cp_height); // see "cs1037lib-buttons.h"
	int dropList_files = CreateDropList(6, image_names, 0, image_load); // the last argument specifies the call-back function, see "cs1037lib-buttons.h"
	int label = CreateTextLabel("Mode:"); // see "cs1037lib-buttons.h"
	int dropList_modes = CreateDropList(2, mode_names, mode, mode_set); // the last argument specifies the call-back function, see "cs1037lib-buttons.h"
	int button_clear = CreateButton("Clear",clear); // the last argument specifies the call-back function, see "cs1037lib-buttons.h"
	int check_box_view = CreateCheckBox("view" , view, view_set); // see "cs1037lib-buttons.h"
	SetWindowTitle("CS1037 Segmentation");      // see "cs1037lib-windows.h"
    SetDrawAxis(pad,cp_height+pad,false); // sets window's "coordinate center" for GetMouseInput(x,y) and for all DrawXXX(x,y) functions in "cs1037lib-window.h" 
	                                      // we set it in the left corner below the "control panel" with buttons
	  // initializing the application
	image_load(0);
	SetWindowVisible(true); // see "cs1037lib-window.h"
	...
	DeleteControl(button_clear);    // see "cs1037lib-buttons.h"
